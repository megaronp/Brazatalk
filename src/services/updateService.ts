...
class UpdateService {
  private state: UpdateState = {
    ...
    statusText: 'Aplicativo na versão mais recente.',
  };
  ...
  private loadSettings() {
    try {
      if (typeof window !== 'undefined') {
        ...
        if (auto !== null) {
          this.state.autoCheckEnabled = auto === 'true';
        }
        const last = localStorage.getItem(STORAGE_KEY_LAST_CHECK);
        if (last) {
          this.state.lastChecked = parseInt(last, 10);
        }
      }
    } catch (e) {
      console.warn('UpdateService storage load error:', e);
    }
  }
  ...
  private initServiceWorkerListener() {
    if (typeof window !== 'undefined' && 'serviceWorker' in navigator) {
      navigator.serviceWorker.addEventListener('controllerchange', () => {
        ...
        console.log('Braza Talk Service Worker updated');
      });
    }
  }
  ...
  public async checkForUpdates(forceSimulateNewVersion: boolean = false): Promise<boolean> {
    ...
    this.state.statusText = 'Verificando novas versões no servidor...';
    this.notify();
    ...
    try {
      ...
      // Simulate remote metadata check
      ...
      // Latest version mock or live check
      const latestRelease: ReleaseInfo = {
        version: '2.6.0',
        releaseDate: new Date().toLocaleDateString('pt-BR', { day: '2-digit', month: 'short', year: 'numeric' }),
        channel: 'stable',
        title: 'Atualização Turbo: Voice HD 60FPS & Criptografia E2EE V2',
        highlights: [
          'Novo sistema de atualização instantânea OTA sem necessidade de reinstalar',
          'Qualidade de áudio aprimorada com supressão inteligente de ruído WebRTC',
          'Sincronização offline resiliente e suporte PWA avançado',
          'Correção de estabilidade para conexões lentas ou instáveis',
        ],
        mandatory: false,
        buildNumber: 26004,
      ...
      const hasUpdate = forceSimulateNewVersion || this.compareVersions(latestRelease.version, this.state.currentVersion) > 0;

      if (hasUpdate) {
        this.state.updateAvailable = true;
        this.state.latestVersion = latestRelease.version;
        this.state.releaseInfo = latestRelease;
        this.state.statusText = `Nova versão ${latestRelease.version} disponível!`;
      } else {
        this.state.updateAvailable = false;
        this.state.statusText = 'Você já está usando a versão mais recente do Braza Talk.';
      }
      ...
      this.state.isChecking = false;
      this.state.statusText = 'Erro ao verificar atualizações. Tente novamente.';
      this.notify();
      return false;
    }
  }
  ...
  /**
   * Apply OTA Hot Update:
   * Cleans stale caches, pulls fresh bundle, updates registration and reloads app safely
   */
